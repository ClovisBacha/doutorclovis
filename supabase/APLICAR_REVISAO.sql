-- ════════════════════════════════════════════════════════════════════════════
-- APLICAR NO SQL EDITOR DO SUPABASE — a fila de REVISÃO
--
...
-- O PROBLEMA
--
-- O 👎 da paciente virava "a IA não soube responder" — mesmo quando ela soube.
--
-- Se a resposta veio de uma entrada APROVADA do médico e estava errada, o
-- sistema mandava aquela pergunta para a fila de lacunas, num card que diz
-- exatamente o contrário do que aconteceu. Pior: ao respondê-la, o médico
-- criava uma SEGUNDA entrada com a mesma pergunta — e a primeira, a ruim,
-- continuava aprovada e continuava competindo na busca. O cérebro ficava com
-- duas verdades sobre o mesmo assunto e nenhuma forma de escolher.
--
-- São duas situações diferentes e precisam de duas filas:
--
--   a IA NÃO SABIA  → lacuna. Ele ensina algo novo.
--   a IA SABIA e errou → revisão. Ele corrige o que já existe.
--
-- E o 👎 não guardava NADA do que foi reprovado: nem o texto da resposta, nem
-- qual entrada a produziu. Sem isso não há o que revisar — só a pergunta, que
-- é a única coisa que não estava errada.
...
--    WHERE table_name = 'brain_feedback' ORDER BY 1;
--
--   -- a fila de revisão de hoje (esperado: vazia num cérebro novo)
--   SELECT question, left(answer, 60) AS resposta, status
--     FROM public.brain_feedback
--    WHERE helpful = false AND status = 'aberta';
