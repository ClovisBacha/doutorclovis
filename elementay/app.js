...
const PRODUCTS = [
  {
    ...
    el: "fogo",
    name: "Jaqueta Brasa",
    desc: "Edição numerada · couro vegetal",
    price: 689,
  },
  {
    id: "fogo-2",
    el: "fogo",
    name: "Vestido Fúria",
    desc: "Fenda assimétrica · seda flamejada",
    price: 549,
  },
  {
    id: "fogo-3",
    el: "fogo",
    name: "Camisa Fagulha",
    desc: "Bordado artesanal · botão de lava",
    price: 329,
  },
  {
    id: "agua-1",
    el: "agua",
    name: "Biquíni Maré",
    desc: "Tecido reciclado do oceano · UV50+",
    price: 219,
  },
  {
    id: "agua-2",
    el: "agua",
    name: "Sunga Corrente",
    desc: "Secagem rápida · cós anatômico",
    price: 149,
  },
  {
    id: "agua-3",
    el: "agua",
    name: "Saída Brisa-do-Mar",
    desc: "Linho leve · trama aberta",
    price: 189,
  },
  {
    id: "terra-1",
    el: "terra",
    name: "Legging Rocha",
    desc: "Compressão gradual · bolso lateral",
    price: 249,
  },
  {
    id: "terra-2",
    el: "terra",
    name: "Top Cordilheira",
    desc: "Alta sustentação · malha respirável",
    price: 179,
  },
  {
    id: "terra-3",
    el: "terra",
    name: "Shorts Trilha Viva",
    desc: "Ripstop flexível · cadarço interno",
    price: 199,
  },
  {
    id: "ar-1",
    el: "ar",
    name: "Moletom Nuvem",
    desc: "Algodão penteado · interior felpado",
    price: 259,
  },
  {
    id: "ar-2",
    el: "ar",
    name: "Camiseta Pluma",
    desc: "Oversized · modal ultramacio",
    price: 129,
  },
  {
    id: "ar-3",
    el: "ar",
    name: "Calça Horizonte",
    desc: "Cós elástico · caimento fluido",
    price: 219,
  },
];
...
const NGO_ICO = {
  livro:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 19V5a2 2 0 0 1 2-2h13v16H6a2 2 0 0 0-2 2zm0 0a2 2 0 0 0 2 2h13"/><path d="M8 7h7"/></svg>',
  ...
  broto:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 21v-8"/><path d="M12 13C12 9 9 7 4 7c0 5 3 7 8 6z"/><path d="M12 13c0-4 3-6 8-6 0 5-3 7-8 6z"/></svg>',
};
const NGOS = [
  {
    id: "semear",
    ico: NGO_ICO.livro,
    name: "Instituto Semear Futuro",
    desc: "Educação e reforço escolar para crianças em situação de vulnerabilidade.",
  },
  {
    id: "mar",
    ico: NGO_ICO.onda,
    name: "Mar Sem Plástico",
    desc: "Limpeza de praias e proteção da vida marinha no litoral brasileiro.",
  },
  {
    id: "abrigo",
    ico: NGO_ICO.casa,
    name: "Casa Abrigo Esperança",
    desc: "Acolhimento e recomeço para famílias em situação de rua.",
  },
  {
    id: "raizes",
    ico: NGO_ICO.broto,
    name: "Raízes Vivas",
    desc: "Reflorestamento e agrofloresta com comunidades rurais.",
  },
];
...
function swatchSVG(el, seed) {
  ...
  const g = `g${uid++}`;
  const r = (n) => {
    // pseudo-aleatório estável por produto
...
function renderCatalog() {
  document.querySelectorAll(".cards[data-element]").forEach((grid) => {
    ...
    grid.innerHTML = PRODUCTS.filter((p) => p.el === el)
      .map(
        (p, i) => `
...
function renderNgoSection() {
  document.getElementById("ong-list").innerHTML = NGOS.map(
    (n) => `
    <div class="cause">
...
function renderCart() {
  ...
  if (!entries.length) {
    box.innerHTML =
      '<p class="cart-empty">Seu carrinho está vazio.<br/>Escolha um elemento e comece por ele.</p>';
  } else {
...
function renderNgoGrid() {
  document.getElementById("ong-grid").innerHTML = NGOS.map(
    (n) => `
...
function goToStep(n) {
  document
    ...
    .forEach((s) => s.classList.toggle("on", +s.dataset.step <= Math.min(n, 3)));
  [1, 2, 3, 4].forEach((i) => {
    document.getElementById(`pane-${i}`).hidden = i !== n;
  });
}
...
function recapHTML() {
  ...
  const items = cartEntries()
    .map(
      ({ product: p, qty }) =>
        `<div class="row"><span class="muted">${qty}× ${p.name}</span><span>${BRL(p.price * qty)}</span></div>`,
    )
    .join("");
  return `${items}
...
function confirmOrder() {
  ...
  const order = {
    ...
    items: cartEntries().map(({ product: p, qty }) => ({
      id: p.id,
      name: p.name,
      price: p.price,
      qty,
...
makeFx(
  ...
  (W, H) =>
    Array.from({ length: Math.min(90, Math.floor(W / 22)) }, () => ({
      x: rnd(0, W),
      y: rnd(0, H),
      r: rnd(0.6, 2.6) * dpr(),
      vx: rnd(-0.09, 0.09) * dpr(),
      vy: rnd(-0.36, -0.06) * dpr(),
...
function fxFogo(canvas) {
  makeFx(
    canvas,
    (W, H) =>
      Array.from({ length: Math.min(70, Math.floor(W / 30)) }, () => ({
        x: rnd(0, W),
        y: rnd(0, H),
        r: rnd(0.7, 2.4) * dpr(),
        vy: rnd(0.25, 0.9) * dpr(),
...
function fxTerra(canvas) {
  makeFx(
    ...
    (cx, W, H, s) => {
      ...
      cx.lineTo(W, H);
      cx.closePath();
      cx.fillStyle = "rgba(46,42,28,.5)";
      cx.fill();
      for (const d of s.dust) {
        d.x += d.vx;
        d.y += d.vy + Math.sin((d.ph += 0.01)) * 0.08 * dpr();
        if (d.x > W + 8) {
          d.x = -8;
          d.y = rnd(0, H);
        }
  ...
  makeFx(
    ...
    (cx, W, H, s) => {
      ...
      for (const c of s.clouds) {
        ...
        cx.scale(1, c.ry / c.rx);
        cx.beginPath();
        cx.arc(0, 0, c.rx, 0, Math.PI * 2);
        cx.fill();
        cx.restore();
      }
      cx.globalAlpha = 0.25;
      cx.strokeStyle = "#5d707f";
      cx.lineWidth = dpr();
      for (let i = 0; i < 3; i++) {
        cx.beginPath();
