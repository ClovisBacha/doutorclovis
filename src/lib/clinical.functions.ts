...
/* ════════════════════════════════════════════════════════════════════════════
   O CICLO DEDICADO DE EXAMES SAIU DAQUI (ago/2026)

   `examesRecebidos`/`imagemDoExame`/`devolutivaDoExame` liam e serviam o
   arquivo que a paciente enviava (`exam_files`, Storage). A plataforma deixou
   de aceitar esse envio — "não vamos ter essa responsabilidade em guardar os
   exames" — e as três funções perderam o único chamador (a aba Exames do
   painel). `EspecieEvento` continua com "exame": os `exam_files` já enviados
   antes desta mudança ainda existem no banco e continuam aparecendo como
   linha do tempo genérica em `clinical_events` — só o visualizador dedicado
   (a imagem/PDF e a devolutiva) saiu.
   ════════════════════════════════════════════════════════════════════════════ */
