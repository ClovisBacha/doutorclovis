...
export const Route = createFileRoute("/cards")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Crie um card lindo da semana da sua gestação para compartilhar no Instagram ou WhatsApp." },
...
const marcos: Record<number, string> = {
  6: "Primeiro batimento", 8: "Forma de feijão 🌱", 12: "Saiu do 1º trimestre 🎉",
  16: "Já dá pra saber o sexo", 20: "Metade do caminho 💛", 24: "Viabilidade ✨",
  28: "Reta final começa", 32: "Quase lá", 37: "Bebê a termo 👶", 40: "Hora de nascer 💖",
};
...
function CardsPage() {
  ...
  async function compartilhar() {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "Minha gestação", text, url }); } catch { /* ignore */ }
    } else {
  ...
  async function baixar() {
    ...
    a.href = url; a.download = `gestacao-${semana}sem.svg`; a.click();
  ...
  return (
    <section className="mx-auto max-w-4xl px-5 py-16">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-primary">Para compartilhar</p>
      ...
      <div className="mt-8 grid gap-8 md:grid-cols-[1fr_1.2fr]">
        <div className="space-y-5">
          <div>
            ...
            <input type="range" min={4} max={40} value={semana} onChange={(e) => setSemana(Number(e.target.value))} className="mt-2 w-full" />
          </div>
          <div>
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={compartilhar} className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">
              <Share2 className="h-4 w-4" /> Compartilhar
            </button>
            <button onClick={baixar} className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-medium">
        ...
        <div
          ...
          style={{ background: "var(--gradient-primary)", color: "hsl(var(--primary-foreground, 0 0% 100%))" }}
        >
